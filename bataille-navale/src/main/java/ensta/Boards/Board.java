...
public class Board implements IBoard, java.io.Serializable{
    ...
    public boolean canPutShip(AbstractShip ship , int x, int y){
        int dx[]= {-1,1,0,0}; //  dx c'est le pas avec le quel avance x qui dépend de l'orientation
        int dy[]= {0, 0,1,-1};  // dy c'est le pas avec le quel avance y qui dépend de l'orientation
        int pos = ship.getOrientation().ordinal(); // indice de l'orientation dans l'enum
        for(int i=0;i<ship.getSize();i++){
            int nx = x+i*dx[pos]; //
            int ny = y+i*dy[pos];
            if(nx<0 || nx>=size || ny<0 || ny>=size || hasShip(nx,ny)){
                return false;
            }
        }
        return true;
    ...
    public void print(){
        ...
        int space = sizeAsString.length(); //  la taille en charactère du taille de la grille
        ...
        for(int i=0;i<size;i++){
            ...
            for(int j=0;j<space-sz;j++)
                line+=" "; //  ajout des espaces qui manque pour les nombre car il y a des nombre de taille 1 ("8" par ex) et d'autre de taille 2 ("11" par ex)
            line+=" ";
